 ...
 *
 * @param {Object} args - Arguments for the display text.
 * @param {string} args.text - The text to display on the LED name badge, including any icons.
...
const executeFunction = async ({ text }) => {
  ...
  try {
    ...
    return {
      error: `An error occurred while displaying text: ${error instanceof Error ? error.message : JSON.stringify(error)}`
    };
  }
};
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          text: {
            type: 'string',
            description: 'The text to display on the LED name badge, including any icons.'
          }
        },
        required: ['text']
      }
